<?php
    ...
    function createUser(string $name , string $username , string $email , string $password){
        ...
        $myfiles=fopen("db.json","w");
        fwrite($myfiles, json_encode($db, JSON_PRETTY_PRINT));
        fclose($myfiles);
    }
    ...
    function kısaOzet(&$film,&$limit, $ozt){

        if(strlen($ozt)>limit){
            echo substr($ozt,0,200)."...";
    ...
    function createBlog(string $title,string $sdescription,string $description, string $imageUrl, string $url , int $isAvtive=0){
        include "ayar.php";
        $query= "INSERT INTO blogs(title ,short_description ,description , imageUrl, url , isActive) VALUES (?,?,?,?,?,?)";
        $result= mysqli_prepare($connection,$query);
        mysqli_stmt_bind_param($result,'sssssi',$title,$sdescription,$description,$imageUrl,$url,$isAvtive);
    ...
    function getBlogsByFilters($categoryId, $keyword, $page){
        ...
        if(!empty ($keyword)){
            $query .= " && b.title LIKE '%$keyword%' or b.description LIKE '%$keyword%' or b.short_description LIKE '%$keyword%'";
        }
    ...
    function editCategory(int $id ,string $categoryname, int $isAvtive){
        include "ayar.php";
        $query= "UPDATE categories SET name = '$categoryname' , isActive = $isAvtive WHERE id = $id ";
        $result = mysqli_query($connection, $query);
